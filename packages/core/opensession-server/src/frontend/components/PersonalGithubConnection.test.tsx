...
const DISCLOSURE_TEXT =
  "Your connection is personal, but the server is shared. Server operators and agents with administrator (root) access can read your GitHub credentials, repository files, and session data.";
...
function status(
  ...
  return {
    ...
    disclosure: { version: "shared-host-v1", text: DISCLOSURE_TEXT },
    repositoryAdmission,
    status: connection,
  };
}
...
describe("personal GitHub card", () => {
  test("shows the server's disclosure in full and keeps connect off until it is accepted", () => {
    ...
    expect(html).toContain("Disclosure version shared-host-v1");
