...
> Your connection is personal, but the server is shared. Server operators and agents running under Open Session's system account or with administrator (root) access can read your GitHub credentials, repository files, and session data. Only connect repositories you trust this server with.
...
7. The internal runtime credential resolver revalidates the exact App/owner/repo/installation/revision and catalog binding on every use, returns a fresh canonical repository name, and verifies either the issuing App's user grant or a numeric repository-narrowed installation token. Projected installation tokens are durably tracked and revoked before disconnect completes. There is no HTTP/MCP token endpoint.
