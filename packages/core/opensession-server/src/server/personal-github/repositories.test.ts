  ...
  const transport: import("./github-api").PersonalGithubTransport = async (
    ...
    if (url.endsWith("/access_tokens")) {
      ...
      if (body.repository_ids) {
        expect(body.repository_ids).toEqual([501]);
...
test("A/B overlapping repo ids cannot cross owner/App/installation/revision; no user fallback", async () => {
  ...
  expect(
    await f.opened.repositories!.resolveCredential(12, b.descriptor, "user"),
  ).toMatchObject({ code: "grant_needs_reconnect" });
...
test("user projection revalidates the issuing grant's numeric identity without installation fallback", async () => {
  const f = await fixture();
  const registered = await f.register();
  if (!registered.ok) throw new Error(registered.code);
  ...
  expect(f.events.filter((event) => event.startsWith("mint:"))).toHaveLength(0);
  f.userOwner(12);
  const rejected = await f.opened.repositories!.resolveCredential(
    11,
    registered.descriptor,
    "user",
  );
  expect(rejected).toMatchObject({ code: "grant_owner_mismatch" });
  expect(JSON.stringify(rejected)).not.toContain("personal-user-token");
  expect(await f.opened.connections.disconnect(11)).toEqual({ ok: true });
});
