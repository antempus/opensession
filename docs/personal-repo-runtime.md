...
The accepted connection disclosure must explain that root-capable operators,
agents and same-uid host processes can access private data. The retired private
...
| Human code turn                     | `user`               |
| Machine/unattended code turn        | `installation-write` |

The broker validates current App/grant state, numeric repository ownership,
...
5. The helper checks the source `runner-host/personal-host.ts` entrypoint, or
   ...
   bounded output. Exact response: `personal-runner-host-v1`. Old helpers,
   source releases or unsupported compiled binaries are rejected.
...
from `expiresAt`, the actual broker token expiry (nullable only for a user grant).
