...
/** Only correspondence is summarized, never arbitrary transcript/tool content. */
export function agentMessageText(entry: TranscriptEntry): string | null {
  const outgoing = outgoingAgentMessage(entry);
  if (outgoing) return outgoing.content;
  const classified = classifyEntry(entry);
  return classified.notice?.kind === "session-notice" ||
