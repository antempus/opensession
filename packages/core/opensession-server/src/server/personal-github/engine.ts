  ...
  return {
    ...
    > {
      return guarded(owner, async () => {
        ...
        if (
          !coordinator ||
          !["user", "installation-read", "installation-write"].includes(kind)
        )
          return deny(
            "credential_denied",
        ...
        if (kind === "user") {
          const grant = await broker.refreshUserGrant(ref);
          if (grant.status !== "current" && grant.status !== "refreshed") {
            if (grant.status !== "missing") await broker.revokeAccess(ref);
            return deny(
              "grant_needs_reconnect",
              "Connect the personal App's user grant.",
            );
          }
          const user = await api.getAuthenticatedUser(grant.grant.accessToken);
          if (
            !user.ok ||
            user.value.id !== owner ||
            user.value.type !== "User" ||
            grant.grant.grantedGithubAccountId !== owner ||
            grant.grant.refreshFailedAt !== null ||
            (grant.grant.expiresAt !== null &&
              grant.grant.expiresAt <= now() + 60_000)
          ) {
            await broker.revokeAccess(ref);
            return deny(
              "grant_owner_mismatch",
              "Personal grant could not be verified.",
            );
          }
          token = grant.grant.accessToken;
          expiresAt = grant.grant.expiresAt;
        } else {
          const jwt = await broker.signAppJwt(ref, Math.floor(now() / 1000));
          if (!jwt)
          ...
          try {
            if (expiresAt === null || expiresAt <= now() + 60_000)
