...
export interface PersonalHostProjection {
  ...
  admitUntil: number;
  /** Actual broker credential expiry; null only for a non-expiring user grant. */
...
export async function validatePersonalHostProjection(
  ...
  if (
    ...
    (record.kind !== "user" && record.expiresAt === null) ||
...
export function personalHostProjection(
  ...
  if (
    !samePersonalRepoBinding(adopted.binding, binding) ||
    (adopted.expiresAt !== null && adopted.expiresAt <= now)
  )
    throw new PersonalRepoRuntimeError();
  return adopted;
}
