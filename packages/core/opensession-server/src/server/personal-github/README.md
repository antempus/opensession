...
runtime API; kinds are `user`, `installation-read`, `installation-write`, selected
...
User mode verifies/refreshes only that App's grant and `/user` numeric identity;
installation modes mint with exactly `[repositoryId]` and narrowed permissions.
No user-to-App or shared/ambient fallback exists. No HTTP/MCP token endpoint.
